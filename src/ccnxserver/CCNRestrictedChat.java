 ...
 * @author Wojtek
 */
public class CCNRestrictedChat {
...
//	}
	 ...
	 */
    public CCNRestrictedChat(CCNChatCallback callback, String namespace, String restrictedName) throws MalformedContentNameStringException {
    	_callback = callback;
        _serverNamespace = ContentName.fromURI(namespace);
    	_namespace = ContentName.fromURI(namespace + restrictedName);  	
    	_namespaceStr = namespace + restrictedName;
        _restrictedName = restrictedName;
	...
	public void listen() throws ConfigurationException, IOException, MalformedContentNameStringException {
                ...
                String textMessage = messager.getStringMessage(_restrictedName, MessageFactory.INIT_MESSAGE);

//                        System.out.println(textMessage);
                //publish server message
		if (_serverNamespace.toString().startsWith("ccnx:/")) {
			UserConfiguration.setDefaultNamespacePrefix(_serverNamespace.toString().substring(5));		
		} else {
			UserConfiguration.setDefaultNamespacePrefix(_serverNamespace.toString());
		} 
                CCNHandle serverReadHandle = CCNHandle.open();
                _readString = new CCNStringObject(_serverNamespace, (String)null, CCNFlowControl.SaveType.RAW, serverReadHandle);
		_readString.updateInBackground(true);
                CCNHandle serverWriteHandle = CCNHandle.open();
		_writeString = new CCNStringObject(_serverNamespace, textMessage, CCNFlowControl.SaveType.RAW, serverWriteHandle);                
		_writeString.save();
//		serverWriteHandle.close();
		...
		_readString = new CCNStringObject(_namespace, (String)null, CCNFlowControl.SaveType.RAW, tempReadHandle);
		_readString.updateInBackground(true); 
		...
		_writeString = new CCNStringObject(_namespace, introduction, CCNFlowControl.SaveType.RAW, tempWriteHandle);
//		_writeString = new CCNStringObject(_namespace, _restrictedName, CCNFlowControl.SaveType.RAW, tempWriteHandle);                
//		_writeString.save();
		...
		_readNameString = new CCNStringObject(_friendlyNameNamespace, (String)null, CCNFlowControl.SaveType.RAW, tempReadHandle);
		...
		_writeNameString = new CCNStringObject(_friendlyNameNamespace, publishedNameStr, CCNFlowControl.SaveType.RAW, tempWriteHandle);
	...
	private final CCNChatCallback _callback;
        private final ContentName _serverNamespace;
	private final ContentName _namespace;
        private final String _namespaceStr;
        private final String _restrictedName;
